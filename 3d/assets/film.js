...
;(async function boot() {
  ...
  try {
    ...
    if (seek !== null) {
      ...
      // The film starts on its own; the score cannot, because no browser begins
      // audio without a gesture. The transport's Sound control carries that —
      // it reads "Sound — click" until one arrives, and the first interaction
      // anywhere on the page starts the cue.
